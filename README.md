...
7. **Fill the toolchain placeholders and set up the workflows.** On the pack path, step 5 already supplied the commands for the first two bullets; **both paths** still do the last three:
...
11. **Protect `main`.** Add a branch protection rule / ruleset requiring the CI workflow to pass before merge, blocking force pushes and deletion, and allowing only rebase merges so history stays linear. Install the rule **after step 13's first green push** (or run steps 5–10 on a branch and merge them via a PR) — a required-status rule rejects a direct push whose CI has never run. Trunk must stay releasable — and on packs whose pipeline ships whatever lands on `main` (e.g. `vercel-csr`), green-CI-before-merge *is* the deploy gate.

    GitHub only offers rulesets and branch protection on a **public** repository or a **paid** plan. On a private repo on GitHub Free, the API returns 403 and the rule cannot be created — treat the merge discipline in `CLAUDE.md` *Development workflow* as the gate until the plan or the visibility changes, and install the rule the day it becomes available.
...
- A security concern: email **adam@cavalry.sg** rather than opening a public issue — see [`SECURITY.md`](SECURITY.md).
- Anything else, including conduct reports: email **adam@cavalry.sg**. Conduct in this project's spaces is governed by [`CODE_OF_CONDUCT.md`](CODE_OF_CONDUCT.md).
