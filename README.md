...
> If you chose a server-first Next.js pack (`nextjs-nestjs-postgres`, `vercel`, or `vercel-ssr`), soften the SPA framing the base ships agnostic: root `CLAUDE.md` "the single-page app" → "the web frontend", and the **What's included** "Frontend SPA" row above → "Frontend (server-first Next.js)". (`vercel-ssr`'s one-app restructure step covers this and more — see its README.) The repo name still encodes "spa" and is immutable — accepted as stale.
