...
| `add-ons/` | Optional capability add-ons — agnostic patterns you opt into at Day-1 (test mode, OTP login, LLM calls, …); concrete bindings come from the active stack pack's appendices or a `bindings.md` inside the add-on. See [`add-ons/README.md`](add-ons/README.md) |
...
6. **Choose your add-ons.** Under `add-ons/`, keep the optional capabilities you want and **delete the directories you don't** — every directory kept is adopted, and the root `CLAUDE.md` points agents at each kept add-on's README. The full inventory is the **Current add-ons** table in [`add-ons/README.md`](add-ons/README.md). Each adopted add-on's concrete bindings come from the active stack pack's appendices or from a `bindings.md` the add-on carries itself — its README says which; if the add-on carries the file, delete the sections for packs you didn't adopt.
...
> If you chose a server-first Next.js pack (`nextjs-nestjs-postgres`, `vercel`, or `vercel-ssr`), soften the SPA framing the base ships agnostic: root `CLAUDE.md` "the single-page app" → "the web frontend", the opening line of `apps/frontend/CLAUDE.md` ("how the single-page app is structured") likewise, and the **What's included** "Frontend SPA" row above → "Frontend (server-first Next.js)". (`vercel-ssr`'s one-app restructure step covers this and more — see its README.) The repo name still encodes "spa" and is immutable — accepted as stale.
