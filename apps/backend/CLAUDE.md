...
- **Secrets stored through the API are write-only:** never return a stored secret on read — expose only a "configured" indicator — and treat a blank value on update as "keep the existing secret", so re-saving a form never clears one the user didn't retype.
- **Same-origin first; CORS is opt-in:** serve the frontend and API from one origin (a proxy or one process) so CORS never applies. If a genuinely cross-origin consumer exists, allow-list exact origins — never `*` with credentials — and keep the allowlist in validated config. The active stack pack binds the serving shape.
- **Sessions in HTTP-only cookies by default:** session/auth tokens travel in signed, HTTP-only, secure cookies unless the active stack pack registers a different mechanism; never issue a session token for the SPA to keep in `localStorage`.
