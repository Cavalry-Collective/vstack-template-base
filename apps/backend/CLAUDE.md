...
- **Same-origin first; CORS is opt-in:** serve the frontend and API from one origin (a proxy or one process) so CORS never applies. If a genuinely cross-origin consumer exists, allow-list exact origins — never `*` with credentials — and keep the allowlist in validated config. (Concrete serving shape: the active stack pack.)
