...
- **Correlation id is never discarded.** The backend returns a correlation id on errors; the shared error-handling path reads it from a failed response, shows it unobtrusively in the user-visible error UI ("Error reference: `<id>`") so it can be quoted in a support report, and attaches it to any client-side error / telemetry report.
