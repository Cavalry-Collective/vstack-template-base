...
2. **User stories** — priority-tagged; each carries acceptance criteria *and how each one will be verified* (the exact command, endpoint call, or screen + states to exercise). For any UI story, name the specific `design/` mockup file(s) it implements (screen, copy, and flow); "matches the referenced mockup" is part of done. If no mockup exists for a new screen, record it under **Open questions** and resolve it before implementation rather than inventing the design.
