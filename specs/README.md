...
- **Add-on programs are reference, not requirements.** An add-on may ship a spec program inside its own directory (`add-ons/<name>/specs/` or a single `SPEC.md`; see `add-ons/README.md`), so deleting the add-on carries it. That program never moves here: this directory holds only the project's actual requirement specs. When the project implements an add-on area, write its requirement spec here, scoped to what the project is building and drawing on the add-on's program. A numbering tool scans only this directory, so feature numbers consumed anywhere else are recorded in root `CLAUDE.md` **Learnings**, the single registry. Don't reuse a consumed number
