...
- **Add-ons carry guidance, not requirement specs.** An add-on README states its capability's coverage checklist and opinionated approach (see `add-ons/README.md`). When the project implements an add-on area, write the actual requirement spec here, scoped to what the project is building and covering that add-on's *Implementation areas*. A numbering tool scans only this directory, so feature numbers consumed anywhere else are recorded in root `CLAUDE.md` **Learnings**, the single registry. Don't reuse a consumed number
