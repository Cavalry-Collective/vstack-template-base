...
9. Bulk update and bulk delete are separate permissions per resource: `contacts:bulk_update` / `contacts:bulk_delete`, and likewise for `companies` and `deals`. This adds `bulk_update` to the RBAC permission catalog (`2026-07-07-compliance-rbac.md`) — an additive amendment to that spec, not a fork.
