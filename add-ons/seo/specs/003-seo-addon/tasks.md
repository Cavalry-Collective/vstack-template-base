...
> **Historical record:** completed tasks below describe deliverables as shipped at 003-time; some were later moved or changed — bindings written into `stacks/*` files were relocated to `add-ons/seo/bindings.md`, the enumeration edits were removed by the folder-isolation relocation (registry row later restored), and a fourth pack (`vercel-ssr`) shipped after the "all three packs" checkpoints below. See the annotation atop [spec.md](spec.md).
