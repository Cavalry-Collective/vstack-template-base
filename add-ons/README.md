...
1. Create `add-ons/<name>/README.md` — the agnostic approach plus its "binds to a stack" and "interactions" sections, per the invariants. A capability too big for SOP-only guidance ships its buildable program as specs in the same directory (`specs/` + program index, or a single `SPEC.md`), written to `specs/README.md` conventions.
