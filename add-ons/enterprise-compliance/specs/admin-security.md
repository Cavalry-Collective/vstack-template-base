...
Give org admins a security-settings surface: session policy and management, password policy, IP allowlisting, API-key management, security notifications, and a per-member security view. A tenant can then enforce its own security posture as validated, audited org policy.

## Scope & ownership

- **Owns:** the org security policy resource (`…/security-policy` and the shared `…/policies` org-policy store), server-side sessions and revocation, password policy and login throttling, IP allowlisting, API keys, security notification emails, and the members-security view. **Consumes:** the RBAC `PermissionChecker` port for API-key scope checks (`rbac.md`); the shared approval flow for guarded policy changes (`maker-checker.md`); MFA enrollment and SSO link status read from those specs' tables; the mail port, stubbed under the test-mode add-on.
- **Phases:** P1 = session policy + session management (S1–S2). P2 = password policy, IP allowlists, API keys, notifications, members-security view (S3–S7). Everything ships behind the program's default-off rollout flag; org-visible enforcement additionally follows the org's own policy values.
...
4. **Password policy** (password-auth orgs only): minimum length configurable ≥ 12; candidate passwords are rejected if found in a breach corpus via a k-anonymity-style check (concrete check supplied by the active stack pack, degradable to allow-with-warning if the corpus is unreachable — never a hard login outage); **no composition rules** (no mandatory symbol/case classes). Login attempts are throttled per account + IP with a temporary lockout after repeated failures (defaults per *Policy keys & bounds*).
...
Policy keys & bounds (requirements 2, 4, 5):

| Key | Default | Bounds / values |
|---|---|---|
| `session_idle_timeout_minutes` | 30 min | 5 min–24 h |
| `session_absolute_lifetime_minutes` | 12 h | 1 h–30 days |
| `password_min_length` | 12 | 12–128 |
| `login_max_failures` / `login_lockout_minutes` | 10 failures / 15 min lockout | bounds validated |
| `ip_allowlist_mode` | `off` | `off` \| `report_only` \| `enforced` |
| `notify_new_device` / `notify_credential_change` / `notify_policy_change` | `true` | boolean |

## User flows

### F1 — Tighten session policy (P1)
1. Admin opens Security settings. 2. Edits idle timeout / absolute lifetime; UI shows bounds. 3. Save → backend validates bounds, applies (or routes to approval), audits. 4. Existing sessions adopt the new limits at their next request evaluation.

### F2 — Revoke a session (P1)
1. A member (own sessions) or admin (any member, via the members-security view) lists active sessions — device/user-agent, IP, created, last seen. 2. Picks one (or "all others") and revokes. 3. Backend marks the session revoked and audits; the revoked session's next request gets `401 SESSION_REVOKED`.

### F3 — Roll out an IP allowlist (P2)
1. Admin adds CIDR entries with labels. 2. Sets mode `report_only`; traffic proceeds while would-block events accumulate. 3. Admin reviews the report (audit-log query, `audit-logs.md`). 4. Switches to `enforced`; if the admin's own IP doesn't match, the save is rejected until re-submitted with explicit confirmation. 5. Non-matching requests now get `403 IP_NOT_ALLOWED`; lockout recovery goes through the Owner break-glass path.

### F4 — Create and rotate an API key (P2)
1. Admin creates a key: name, scopes (UI offers only the creator's own permissions), optional expiry. 2. Secret displayed once with a copy control and a "you won't see this again" notice. 3. Later: rotate → new secret shown once, old secret valid for the overlap window, then rejected. 4. Revoke → key rejected immediately.

### F5 — Lockout after repeated failures (P2)
1. Repeated failed logins hit the throttle. 2. Further attempts get `429 LOGIN_TEMPORARILY_LOCKED` until the lockout lapses. 3. The lockout is audited; the member gets a security notification.
...
- `org_policy:read` / `org_policy:update` — read / edit the policy (session, password, notifications, IP allowlist and its mode). `api_keys:read` / `api_keys:create` / `api_keys:update` (rename, scopes within own set, expiry, rotation) / `api_keys:revoke`.
- `members:read` — list any member's sessions and the members-security view; `members:update` — revoke another member's session. Members need no extra permission for their own sessions/notifications.
...
| `GET`/`DELETE …/members/me/sessions[/{sessionId}]` | List / revoke own sessions | authenticated member | items: `{ id, createdAt, lastSeenAt, ip, userAgent, current }`; revoke returns `204` |
| `GET`/`DELETE …/members/{memberId}/sessions[/{sessionId}]` | List / revoke a member's sessions | `members:read` to list, `members:update` to revoke | same item shape; revoke returns `204` |
...
New tables (snake_case; reversible up/down migrations per `db/CLAUDE.md`; every table org-scoped with `id` PK, `organisation_id` FK indexed, `created_at`/`updated_at`). No new instance-global tables; `mfa_enrolled` / `sso_linked` for the security overview are read from the MFA and SSO specs' tables, not duplicated here.

**`org_security_policies`** — one row per org (`organisation_id` unique): the columns in *Policy keys & bounds*, all bounds enforced in the policy schema. **`ip_allowlist_entries`** — `cidr` (validated CIDR text), `label`, `created_by` (member FK); unique `(organisation_id, cidr)`.

**`member_sessions`** — indexed on `(member_id, revoked_at)` for active-session listings:

| Column | Notes |
|---|---|
| `member_id`, `ip`, `user_agent` | FK, indexed; session display metadata |
| `token_hash` | **hashed** — the session token is never stored in clear |
| `last_seen_at` / `absolute_expires_at` | drive the idle/absolute checks |
| `revoked_at` / `revoked_by` | nullable; member FK |

**`api_keys`** — unique `(organisation_id, name)` among non-revoked keys:

| Column | Notes |
|---|---|
| `name` / `key_prefix` | short recoverable prefix for display |
| `secret_hash` | **hashed**, strong KDF via the stack pack |
| `scopes` | validated catalog permission keys |
| `created_by`, `expires_at`, `last_used_at`, `rotated_from_api_key_id`, `revoked_at` | member FK; nullable expiry; coarse last-use (see Implementation notes); rotation self-FK; revocation timestamp |
...
| `security.ip.blocked` / `security.ip.flagged` | Request denied in `enforced` mode / would-block in `report_only` (flagged events deduplicated per actor+IP per hour to bound volume) | `outcome: denied` / `failure`; source IP |
...
| `security.api_key.created` / `security.api_key.updated` / `security.api_key.rotated` / `security.api_key.revoked` | Key lifecycle (create, name/scopes/expiry change, secret rotation, revocation) | `after`: name, prefix, scopes, expiry; `before`/`after` diff on update; overlap end on rotation |
| `security.api_key.change_denied` | Key mutation blocked (permission, scope-subset) | `outcome: denied` |

## Implementation notes

- Lives in a `security` module; policy entity + bounds rules in the domain, use cases in the service ring, session/key stores as repo adapters, routes + guards in the controller ring. Each endpoint's permission is a controller-ring guard; state-dependent rules (scope-subset, self-lockout confirmation, own-vs-other session) re-checked in the use case.
- **Session enforcement** is a controller-ring aspect on all authenticated routes: resolve the session record, reject if revoked or past idle/absolute limits, update `last_seen_at` (write-behind/coarsened to avoid a hot write per request; supplied store per the stack pack). Policy values are read through a cached org-policy port, invalidated synchronously on policy update (same strategy as the RBAC permission cache). Revocation is authoritative server-side state — no reliance on client-side token expiry (add-on principle: sessions are server-revocable).
- **IP allowlist evaluation** sits in the same edge aspect, after authentication resolves the org: `enforced` blocks with `403 IP_NOT_ALLOWED`; `report_only` records the would-block observation and proceeds. Applies to session and API-key traffic alike. CIDR parsing/matching uses an established library (never hand-rolled). The **self-lockout guard** is a use-case rule: on saving `enforced` mode or editing entries while enforced, match the request's IP against the resulting list; mismatch without `confirmCurrentIpMismatch` → `409 IP_SELF_LOCKOUT_UNCONFIRMED`.
- **API keys**: secret generated server-side, returned once, stored only as `secret_hash`; lookup by `key_prefix` then constant-time hash compare. Rotation creates a successor row linked via `rotated_from_api_key_id`; the predecessor's `expires_at` is set to the overlap end. Scope-subset check runs against the *creator's* effective permissions via the RBAC `PermissionChecker` port at create/update time (not re-evaluated per request — the key's own scopes are authoritative afterwards). Revoking the creator does not silently widen or invalidate existing keys; their scopes stand until reviewed (surfaced in the members-security view work).
- **Throttle/lockout** state lives in the rate-limit store named by the stack pack; counting is per `(account, ip)`; concurrency-safe increments (atomic ops, not read-modify-write). The breached-password check is k-anonymity-style: the full candidate never leaves the backend (req. 4).
- **Idempotency**: `PUT …/security-policy` is a full replace — replays are no-ops and no-op saves emit no audit event; session revocation is idempotent (`204` on already-revoked). All queries org-scoped; other orgs' sessions/keys are `404`s. One background sweep job deletes expired/revoked session rows past a grace window and flips expired API keys — housekeeping only, no policy decisions in the job.
...
| Session past idle/absolute limits; revoked session presented | 401 | `SESSION_EXPIRED`; `SESSION_REVOKED` |
...
| Password shorter than the org minimum; password found in the breach corpus | 400 | `PASSWORD_TOO_SHORT`; `PASSWORD_BREACHED` |
...
| Expired / revoked API key presented | 401 | `API_KEY_EXPIRED` / `API_KEY_REVOKED` |
| Session/key id not found (or other org's) | 404 | `SESSION_NOT_FOUND` / `API_KEY_NOT_FOUND` |

## Notes & decisions

- **IP allowlisting cannot skip `report_only`:** the mode ladder, the self-lockout guard, and Owner break-glass together bound the blast radius of a bad rule.
- UX: screens are Security settings (session policy, password policy, IP allowlist with mode switch + confirmation dialog, notification toggles), My sessions (under account settings), Members-security view, and API keys (list, create with one-time secret modal, rotate, revoke). All need loading/error/empty states; destructive actions (revoke, enforce allowlist) need confirmation; the one-time secret display is copyable, never re-fetchable, and the UI states this explicitly. Session and IP checks run on every request — the edge aspect must stay cache-backed and O(1)-ish; no per-request policy-table scan.
