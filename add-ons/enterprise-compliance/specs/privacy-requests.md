...
Give org admins a per-organisation registry and tooling to fulfil data-subject requests — access, portability, erasure, rectification, restriction/objection — against their CRM contacts. Requests carry due-date tracking and completion evidence fit for a regulator. Access requests produce machine-readable export packages; erasure is hold-aware, anonymises in place where records must persist, and feeds a suppression list that blocks resurrection.

## Scope & ownership

- **Framing:** our customers are the **controllers**; their CRM contacts are the **data subjects** (GDPR arts 15–21; Singapore PDPA access and correction obligations). The product is the processor's tooling for the controller.
- **Owns:** the privacy-request registry and state machine, subject locate, DSAR packages, erasure orchestration, the anonymisation rule, the suppression list, and processing-restriction flags.
- **Consumes:** the retention spec's deletion lifecycle — hard deletion runs through its purge machinery, and subject locate uses its explicit `includeDeleted` capability (`retention-deletion.md`); package storage, download-link minting, and TTL mechanics from `export-bulk-controls.md`; encryption and the per-org suppression salt from `encryption.md`; the shared approval flow for guarded erasure (`maker-checker.md`).
- **Not here — vendor-level path for members:** requests by our users about their own member/account data are not handled in this registry. Account settings cover self-service access/rectification/deletion; anything beyond goes through vendor support under the vendor's own privacy policy. Product work here is limited to linking that path from account settings.
- **Phases:** P1 = registry + subject locate, packages, hold-aware erasure (S1–S3). P2 = suppression list, restriction/objection, rectification evidence (S4–S6).
...
### F1 — Register and triage a request (P1)
...
### F2 — Fulfil an access/portability request (P1)
...
### F3 — Fulfil an erasure request (P1)
...
### F4 — Import hits the suppression list (P2)
...
- `privacy_requests:read` — list/filter requests by type, status, assignee, due date; overdue requests surfaced; per-request detail with scope, history, and evidence.
- `privacy_requests:manage` — subject locate, create requests, assign, shorten due dates, record evidence, complete/reject, generate packages and download links, apply/clear restriction flags, view suppression entries (hashes and source request only — plaintext identifiers are never displayable).
- `privacy_requests:execute` — start erasure jobs; remove a suppression entry (rare, e.g. a subject re-consents) — both audited, erasure maker-checker eligible. A separate verb from `manage` per the program's destructive-verb rule.
...
| `privacy.package.link_issued` / `privacy.package.downloaded` | Download link minted / link used | `expires_at`; target = package |
...
- **Background jobs** (runner supplied by the active stack pack), all idempotent, batched, resumable: the *package builder* walks the scope snapshot per class, streams JSON + CSV into encrypted storage, and checkpoints per class/batch so a re-run after interruption resumes without duplicating package content; the *erasure job* re-validates holds at the start of every run (not just enqueue), then per batch hard-deletes via the retention purge machinery or anonymises in place per the domain rule, stamps `disposition` on each scope row (the idempotency marker — a resumed run skips stamped rows), and finally writes suppression entries and completes the request; the *package expiry* job deletes expired packages from storage.
...
- Packages contain full subject PII: stored encrypted at rest (encryption spec), reachable only via short-lived minted links, every issuance and download audited; `storage_ref` never leaves the backend. Audit envelopes for this area carry identifiers and counts, never subject payloads (program redaction rules).
...
- **Salted suppression hashes (req. 7):** the per-org salt means the list cannot be reversed into a contact database, and cross-org correlation of the same subject is impossible.
- **Random-token anonymisation (req. 6):** placeholders are never derived from the original value, so no offline dictionary attack recovers the subject.
- **Subject search is gated by `privacy_requests:manage`, not plain contact read,** and audited per search — it spans soft-deleted rows.
- UX: screens are **Privacy requests** (registry list with overdue highlighting), **New request / subject locate** (search + match confirmation), **Request detail** (status, scope, package, evidence, erasure/restriction actions), and **Suppression list** (read-only table + audited remove). All need loading/error/empty states; lists paginate. Package build and erasure are async — the UI polls and never blocks; erasure actions carry typed confirmation and, where org policy requires, the maker-checker approval banner.
- Performance: subject search must answer interactively (indexed lookups on normalised email/phone); the suppression check adds one hash lookup per import row and must not dominate import throughput.
- Security: no subject PII in URLs, logs, or audit payloads; download links are the only egress path for packages.
...
- Vendor-level DSARs from our own members beyond the account-settings link noted in Scope & ownership.
