...
Make deletion a governed lifecycle: soft delete → grace period → irreversible hard delete with a stated blast radius. Add per-record-class retention policies, legal holds that override every deletion path, and org offboarding with crypto-shredding. Customers can then meet GDPR/PDPA erasure and retention obligations with evidence.

## Scope & ownership

- **Owns:** the deletion lifecycle for CRM data — every other area spec references it rather than redefining it. That covers soft delete, the recycle bin, the grace period, purge and retention sweeps, legal holds, and org offboarding.
- **Consumes:** `BACKUP_RETENTION_DAYS` from `backup-dr.md` (requirement 4 there owns the value); the encryption module's key-destruction port for crypto-shredding (`encryption.md`); the shared approval flow (`maker-checker.md`); the export-controls artifact TTL for blast-radius bounds (`export-bulk-controls.md`).
- **Used by:** `privacy-requests.md` — erasure runs through this spec's purge machinery and its `includeDeleted` capability. Audit-event retention is **not** governed here; it belongs to `audit-logs.md`.
- **Phases:** P1 = soft delete + recycle bin, purge, grace period, retention policies (S1–S4). P2 = legal holds, org offboarding (S5–S6).
...
5. Hard deletion covers the full **blast radius**: primary rows are deleted by the purge job; search-index entries are removed in the same run; caches and generated export files containing the record expire via their own bounded TTLs; backups age out per the backup retention window — `BACKUP_RETENTION_DAYS`, default **35** (stated verbatim per `backup-dr.md` requirement 4, which owns the value). **Documented customer commitment: erasure completes in the primary within grace + 24h, and in backups within the backup retention window — both stated in customer-facing documentation.**
6. Per-record-class **retention policies** (contacts, companies, deals, activities, notes) auto-delete records not updated for N days. Off by default; N validated against declared bounds; enabling requires a fresh preview (req 7).
...
### F2 — Configure the grace period (P1)
...
### F3 — Enable a retention policy (P1)
...
### F4 — Create and release a legal hold (P2)
...
- `retention:read` — view retention settings, policies with last preview, legal-hold list, purge-run history. `retention:update` — change grace period and policies, run previews, enable/disable (maker-checker eligible). `retention:hold` — create and release legal holds.
- `records:restore` — use the recycle bin. `records:purge` — request early purge, separate from restore per the program's destructive-verb rule.
...
- **Background jobs** (runner supplied by the active stack pack), all idempotent, batched, resumable via `purge_runs.checkpoint`: the *purge sweep* (hard-deletes grace-expired, non-held rows; deleting an already-deleted id is a no-op), the *retention sweep* (soft-deletes policy-matching, non-held rows as actor `system`), and the *offboarding purge* (freezes, waits out the window, purges all classes, then calls the encryption module's key-destruction port).
...
- **Key destruction shrinks the backup exposure window:** destroying the org's DEK at offboarding renders backup copies of field-encrypted data unreadable (crypto-shredding), so erasure does not wait on backup ageing alone; mechanics per the encryption spec.
- UX: screens are **Retention settings** (grace period, per-class policies with preview results), **Recycle bin** (filterable list, restore/purge actions), and **Legal holds** (list, create, release). Each needs loading/error/empty states; recycle bin and preview counts paginate. All destructive actions require typed confirmation in the UI.
- UX: preview and purge run asynchronously — the UI polls and never blocks on a sweep.
- Performance: sweeps are batched and off-peak-schedulable so they never lock hot CRM tables; index and query plans for `deleted_at` filters verified before GA.
