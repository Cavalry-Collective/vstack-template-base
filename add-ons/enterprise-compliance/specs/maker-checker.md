...
Provide one generic dual-control (maker-checker) framework: a durable intent is captured instead of executed, decided by a different member, and executed with re-validation. Every guarded capability in the program routes through it, so organisations can require a second pair of eyes on their most dangerous actions without any feature growing its own approval fork (the add-on SOP mandates the shared flow).

## Scope & ownership

- **Owns:** the approval framework — the guardable-action-type catalog, approval requests and their state machine, decision rules 5–11, the approvals inbox, per-org approval policy, and the `202 APPROVAL_REQUIRED` contract that guarded endpoints elsewhere reference.
- **Consumes:** the RBAC `PermissionChecker` port (`rbac.md`); the base notification path (in-app + email, stubbed under test-mode). **Used by:** every spec whose actions appear in the catalog — each owns its action's semantics and registers an executor; this spec owns only the shared framework.
- **Phases:** the framework is **P2** per the program index — no P1 stories here; nothing in the program MVP depends on it. Notifications are P3 (S5).
...
### F1 — Enable dual control (P2)
1. An org admin opens Security settings → Approvals. 2. Sees the catalog with per-entry toggles (and threshold fields for `export.large` / `bulk.delete.large`), all off by default. 3. Enables entries; backend validates and stores per-entry policy, audits `approvals.policy.updated`. 4. From the next request onward, matching actions are guarded.

### F2 — Guarded action captured (P2)
1. A member initiates a guarded action (e.g. assigns the Admin role while `rbac.role.assign_admin_or_owner` is enabled). 2. The owning use case asks the approval seam whether policy guards this action type at this magnitude; it does. 3. The use case persists the intent (`pending`) and performs no state change; the endpoint returns `202` with the request resource. 4. Approvers are notified; `approvals.request.created` is audited. 5. The initiator sees the request under "My requests" and may cancel it.
...
### F4 — Reject (P2)
1. As F3, but the approver rejects and must supply a reason. 2. Nothing executes; `approvals.request.rejected` is audited; the initiator is notified with the reason.

### F5 — Race, expiry, cancellation (P2)
1. Two approvers decide simultaneously: one transition wins; the other gets `409 INTENT_ALREADY_DECIDED` and the UI refreshes to the settled state. 2. A request untouched for the expiry window is auto-rejected by the sweep (`expired`, audited, initiator notified). 3. An initiator cancels a pending request; it leaves the inbox as `cancelled`.
...
- `approvals:read` — review the inbox and request history; initiators always see their own requests without it. `approvals:decide` — decide requests.
- `org_policy:read` / `org_policy:update` — read / set the per-entry approval policy and thresholds. Every request and decision is traceable in the audit viewer with `audit_logs:read`.

| Method & path | Purpose | Permission | Notable fields |
|---|---|---|---|
| `GET …/approval-requests[/{requestId}]` | Inbox / history / detail | `approvals:read`; initiators always see their own | filters `status`, `actionType`, `initiatedBy`; items: `{ id, actionType, summary, status, initiator, decidedBy, expiresAt, createdAt }`; detail adds `parameters` (raw, redacted per governed-field rules), `decisionReason`, `failureReason` |
...
New tables (snake_case; reversible up/down migrations per `db/CLAUDE.md`; `id` PK, `organisation_id` FK indexed, `created_at`/`updated_at`; all sweep/filter columns indexed). Guarded resources in other specs carry a nullable `approval_request_id` FK (e.g. `export_jobs`, `bulk_action_intents`) — owned by those specs. First, **`approval_requests`**:

| Column | Notes |
|---|---|
| `action_type` | validated against the catalog |
| `parameters` / `summary` | serialized inputs the executor needs, schema-checked per action type on write, **never containing secret material** (e.g. `api_key.create` captures key name/scopes, the secret is generated only at execution); human-readable summary generated at capture |
| `initiated_by_actor_type` / `initiated_by_actor_id` | `user` \| `api_key` |
| `status` | `pending` \| `approved` \| `rejected` \| `expired` \| `cancelled` \| `failed` |
| `decided_by` / `decided_at` / `decision_reason` / `failure_reason` | nullable; member FK indexed; the use case requires a reason for rejections; failure set when re-validation/execution fails after approval |
| `expires_at` | indexed for the sweep |

A DB check encodes rule 6: `decided_by IS NULL OR initiated_by_actor_type <> 'user' OR decided_by <> initiated_by_actor_id`; the API-key-ownership variant is enforced in the use case.

**`approval_policies`** — unique `(organisation_id, action_type)`; a missing row means disabled:

| Column | Notes |
|---|---|
| `action_type` | catalog-validated |
| `enabled` / `threshold` | boolean, default false; nullable integer — only for thresholded action types, bounds validated |
| `expiry_days` | default 7, bounds 1–30 |

## Audit events

Emitted via the shared `record()` in the service ring, same transaction as the change. On execution, the guarded action's **own** event (e.g. `rbac.role.assigned`, `export.contacts.completed`) is emitted by its owning module with `context.approval_request_id` and the approving actor — the audit trail links intent, decision, and effect.
...
| `approvals.request.approved` / `approvals.request.rejected` | Approver decided | decider; optional reason on approval, required reason on rejection |
| `approvals.request.cancelled` / `approvals.request.expired` | Initiator cancelled / expiry sweep auto-rejected | actor `system` for expiry |
...
- Permission checks go through the RBAC module's `PermissionChecker` port; edge guards assert `approvals:read`/`approvals:decide`, and rules that depend on state (initiator ≠ decider, expiry, status) are enforced again in the use case. All queries org-scoped; a request id from another org is `404`. Request-detail rendering redacts governed fields per the program envelope rules.
...
- **Deliberate recursion:** while `org_policy.update` is enabled, changing approval policy itself routes through an approval — dual control cannot be switched off unilaterally and silently. Policy changes are also audited.
- **Re-validation (rule 9) closes the time-of-capture/time-of-execution gap:** stale targets, changed thresholds, or revoked permissions fail closed with `failed`, never a partial execution. Bounded expiry (rule 11) prevents zombie intents from executing long after context changed.
- **Self-approval is blocked at two levels** — the DB check and the decision use case (which also blocks a member deciding a request their own API key initiated). No UI-only enforcement.
- UX: screens are the Approvals inbox (pending list + status filters, request detail with summary, raw parameters, decide dialog with reason field), a "My requests" view with cancel, and an Approvals section in Security settings (catalog toggles + thresholds). All need loading/error/empty states; the inbox's empty state explains dual control is opt-in; decision buttons are disabled for the viewer's own requests (server remains authoritative).
- Performance: a pending-approvals badge count for `approvals:decide` holders is one cheap indexed count on `(organisation_id, status)`; capture adds a single insert to the guarded path — no measurable latency budget change.
