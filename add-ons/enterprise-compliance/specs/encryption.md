...
Encrypt by default and manage keys deliberately. TLS covers every external interface and platform encryption covers data at rest. Designated high-sensitivity values get application-layer (field-level) encryption under a per-organisation envelope-key hierarchy, which gives blast-radius isolation and crypto-shredding on offboarding. Rotation is a stated, rehearsed, audited procedure, and no crypto is hand-rolled (root *Don't reinvent*).

## Scope & ownership

- **Owns:** the KEK/DEK envelope hierarchy, the definitive field-level encryption list (requirement 3), the hash-vs-encrypt rule, DEK/KEK rotation, the crypto-shredding contract, sensitive custom fields (P2), and `org_encryption_keys`.
- **Consumes:** the stack pack's KMS and vetted crypto library; `infra/` runbooks for KEK rotation.
- **Used by:** `sso-identity.md` and `mfa.md` (secret/seed encryption), `privacy-requests.md` and `export-bulk-controls.md` (encrypted artifacts, suppression-hash salt), and `retention-deletion.md` — its offboarding use case is the only caller of key destruction and relies on this spec's crypto-shredding contract.
- **Phases:** P1 = TLS, at-rest, field-level encryption, crypto-shredding + KEK rotation (S1–S4). P2 = on-demand DEK rotation, sensitive custom fields (S5–S6). P3 = BYOK (S7).
...
7. **Sensitive custom fields (P2):** an org admin can mark a custom-field definition `sensitive`. Sensitive field values are field-level encrypted with the org's DEK, **excluded from search indexing**, and **excluded from plaintext exports** (export surfaces show a redaction marker; export-bulk-controls spec governs export gating). Marking is one-way; unmarking is rejected with `SENSITIVE_FIELD_IMMUTABLE`.
8. **KMS/decrypt access is audited at the adapter level for governed operations** — one event per governed operation (a job run, a DSAR package build, an admin secret retrieval), never per row.
...
### F1 — Mark a custom field sensitive (P2)
...
2. The API confirms the change; unmark attempts are rejected with `SENSITIVE_FIELD_IMMUTABLE` (req. 7).
...
### F3 — Org key destruction, crypto-shredding at offboarding (P1)
...
2. It invokes the encryption service's destroy operation (system actor; no standalone endpoint — see Notes & decisions).
...
- Key destruction has **no direct endpoint** — it is invoked only by the retention-deletion offboarding use case (F3).
...
- Unwrapped DEKs may be cached in memory per org with a bounded TTL to avoid per-row KMS round-trips; cache eviction on rotation completion and destruction is mandatory. Plaintext DEKs are never persisted and never land in crash dumps by policy (stack pack binds process hardening).
...
- Key material never appears in logs, audit payloads, error messages, or API responses — identifiers and versions only (program criterion 5). Sensitive custom-field values must not leak via search indexes, plaintext exports, logs, or audit diffs — the redaction marker is the only representation outside the read path.
- Config: KMS endpoint/KEK identifier are validated env config, injected inward as values. SSRF/secret-handling/session baselines inherit from `apps/backend/CLAUDE.md`; nothing here relaxes them.
...
| Access touches data whose org DEK was destroyed | 409 | `ORG_KEY_DESTROYED` (status choice: Notes & decisions) |
...
- **`ORG_KEY_DESTROYED` is a 409:** chosen from the base status set — the resource state conflicts irrecoverably with the request. Revisit if the base adopts `410 Gone` (open question below).
- **Sensitive marking is one-way (req. 7):** unmarking would require a decrypt-and-reindex backfill and silently downgrade protection — rejected.
- **No standalone destroy endpoint (F3):** the retention-deletion offboarding lifecycle is the only caller, so exposing a route would add unguarded surface for no consumer (YAGNI). Destruction runs only behind that spec's grace-period and legal-hold gates; its audit event is the durable proof.
- **Aggregate KMS auditing (req. 8):** per-row events would be noise that drowns the audit trail, so events aggregate at the operation boundary.
- **Hash-vs-encrypt is a hard review rule (req. 3):** anything only ever *verified* is hashed; encryption of such a value is a defect.
- UX: screens touched are custom-field settings (sensitive flag + irreversibility warning, P2) and org security settings (read-only key-status card, P2); the trust page states the posture (trust-transparency spec). Rotation shows pending/in-progress/completed/failed on the key-status card; sensitive-field backfill shows an in-progress indicator until search de-indexing completes. Redaction markers, not blanks, wherever a sensitive value is withheld — a blank reads as "no data" and corrupts user trust in exports.
- Performance: field-level encryption costs a mapper round-trip per row — acceptable because the designated fields are low-volume (secrets, seeds) or read singly (sensitive custom fields); DEK caching keeps KMS calls off the request path. Sensitive fields are excluded from search by design, not by perf necessity.
