...
Give every organisation durable, reviewable evidence of who did what. One append-only audit store implements the index's event envelope, and every other area writes to it through a single shared `record()` call. Org admins can search, inspect, and export their events, with retention they configure inside compliance bounds.

## Scope & ownership

- **Owns:** the `audit_events` store, the shared `record()` call, the denied/failed-outcome convention (requirement 4), the admin audit viewer, audit-event retention (`audit_retention_days`), and the meta-events in the table below.
- **Consumes:** export-job mechanics (job resource, status polling, object-storage delivery, size/rate bounds) from `export-bulk-controls.md`; the retention setting rides the org-policy resource owned by `admin-security.md`.
- **Used by:** every other area spec — each emits its events through `record()` and enumerates them in its own audit-events table. This area builds first (index build-order note).
- **Phases:** P1 = store + `record()`, viewer, view auditing (S1–S3). P2 = export and configurable retention (S4–S5). P3 = SIEM streaming (S6).
...
2. All events are emitted through one shared `record()` call in the service ring, invoked by the use case that owns the change. **`record()` participates in the caller's transaction**: if the business transaction commits, the event is recorded; if it rolls back, no event exists.
...
9. The events listing uses **cursor pagination** (a documented per-endpoint decision per the backend contract).
...
### F3 — Configure audit retention (P2)
...
- Export is gated by `audit_logs:export`, separate from `audit_logs:read` per the index's exfiltration rule.
...
- **Export job:** async via the shared job runner; streams the filtered range to object storage (port per stack pack) — never buffers the full range in memory. Idempotent per job id; re-running a failed job overwrites its own partial artifact. Artifacts are downloadable only via short-lived signed URLs (mechanics per the export-controls spec / stack pack).
...
- **Same-database tradeoff (requirement 2):** `record()` commits atomically with the business change, so the audit store must live in the same transactional database as the business data for v1. External/streamed sinks are async consumers layered later (see S6).
- **Cursor pagination (requirement 9):** an offset `COUNT` per request is impractical at audit volume, so the listing is keyset-only.
- UX: screens are the **audit log viewer** (filter bar + list + detail drawer), the **export dialog** (in the viewer), and the **audit retention field** (in the org security-settings screen). Each has loading/error/empty states; empty = "no events match", never an error. Desktop-first admin surface; the drawer must handle large diffs (scroll within, no page overflow).
- Performance: list queries are keyset-indexed; no `COUNT`; p95 list response is expected interactive (<1s) at millions of rows per org.
- Security: no event payload is ever rendered unescaped.
