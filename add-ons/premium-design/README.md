...
- **Elevate presentation, never invent facts — and ship no unfinished surface.** Sharpening weak headings, adding supporting microcopy, and turning flat lists into visual sections (stats, process, comparison, timeline) is in scope; fake testimonials, invented metrics, and misleading claims are not. Placeholder-looking sections, missing empty/loading states, and stock defaults are ship-blockers on a premium surface — finish the section or cut it.

## Verify

The craft review is a per-screen gate. Before a premium screen is done, render it (base rule: observed, not inferred) at the primary form factor *and* mobile, and walk this list — a weak answer is rework, not a note:
- first-screen impact
- typographic rhythm, hierarchy, and spacing consistency
- palette sophistication and contrast; button, card, and section polish (sections visually distinct, not stacked boxes)
- hover/focus refinement; animation taste and cost
- mobile intentionality; zero rough edges
...
- **Base *Design guide*, *Visual quality bar*, *Accessibility baseline*** (`apps/frontend/CLAUDE.md`) — this add-on raises the target, never loosens the floor: every never-violate gate (tokens only, archetypes, surface ladder, reuse-first) and the accessibility baseline (contrast, focus visibility, reduced motion) still hold. Premium is achieved *through* tokens, not around them; a premium treatment that fails contrast is a defect, not a tradeoff.
