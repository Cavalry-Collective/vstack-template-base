...
Leave the API project's framework unset. Vercel detects Fastify from the entrypoint filename (`backend.md` → *Entrypoint*), so pinning a framework or a function runtime only creates drift.
