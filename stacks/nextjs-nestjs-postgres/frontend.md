...
- **Base says:** the base frames `apps/frontend` as a client-rendered SPA (`apps/frontend/CLAUDE.md` → *Versioning / build identity*: "A deployed SPA is cached"; and the `-spa` repo name). **In this stack:** it is server-first Next.js App Router, not a client-rendered SPA — the State, boundary, primitives, and versioning bindings all inherit from this one resolution. **Because:** the chosen stack is App Router. **Concretely:** default every component to server; add `'use client'` only at an interaction leaf. (The repo name stays stale by design.)
