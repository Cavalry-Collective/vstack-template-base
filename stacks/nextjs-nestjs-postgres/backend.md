...
- Version-sensitive bindings (the Prisma config paths, `migrate diff` flags, the Babel decorator recipe) assume the majors current when written — re-verify them on any major upgrade.
...
- **multi-tenancy** (`add-ons/multi-tenancy/`): a `SharedModule` guard resolves the `organisationId` route param, validates authenticated → member → organisation active (membership cached per member, synchronously invalidated, bounded TTL), and seeds the id into the ALS request context; use cases receive it as a plain value. Tenant-owned Prisma models carry `organisationId` with `@@index` and organisation-scoped `@@unique([organisationId, …])`; repos expose only methods that take the organisation id as a required argument. Optional defence-in-depth: Postgres row-level security with a per-transaction `SET LOCAL` tenant setting inside the `TransactionRunner` boundary. Object storage keys are `organisations/{organisationId}/…`, read through an authorised path that re-checks membership.
