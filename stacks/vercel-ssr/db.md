...
Binds the base `db/CLAUDE.md` (+ the repo ring of the relocated backend contract) to **Postgres** — **Neon** (serverless, via the Vercel marketplace) in production, a fixed-name Docker container locally — with **node-pg-migrate** for migrations and **`pg`** (node-postgres) as the query layer. Read the base files first.

**Scope.** This file owns migrations, seed, schema conventions, and the repo-ring query/pool/transaction mechanics. The pool and `withTransaction` live in the db aspect (`./backend.md` → *Aspects*); provisioning lives in `./infra.md`.
...
- Base schema conventions apply unchanged (`db/CLAUDE.md` *Schema conventions*) — bound here: `created_at`/`updated_at` as `timestamptz` defaulted `now()`; FK/filter indexes declared explicitly (Postgres does not auto-index FKs); money as integer minor units or `numeric(p,s)`; unique-violation → domain conflict.
...
- **One `pg.Pool` per process**, created by the db aspect with `max: DB_POOL_MAX`; repos receive it via the container — never construct their own client.
...
- **Mappers at the boundary:** each repo translates rows ↔ domain objects; a raw row (snake_case fields) never crosses inward — and never reaches a Server Component or the client. Select explicit column lists where a subset suffices — no reflexive `SELECT *`.
...
**Deploys do not run migrations.** Vercel builds and ships the app; nothing runs `migrate`. A migration is therefore a **manual step you run before the push that needs it**, against the target Neon branch:
...
- **Run it before you push the code that reads the new schema.** A push deploys the app on Vercel's pipeline without waiting for any migration — so migrate, *then* push. Keep each migration **backward-compatible** (expand → migrate → contract, base `db/CLAUDE.md`) so the brief window where the previous deployment meets the new schema — or the new deployment meets the old schema — never breaks.
