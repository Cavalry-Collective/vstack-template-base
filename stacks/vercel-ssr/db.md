...
Use Postgres 16 locally, Neon in deployed environments, node-pg-migrate for migrations, and `pg` for the server repository ring.
...
- Keep the default transaction. Disable it only for DDL that requires it and explain why.
...
- Create one `pg.Pool` per serverless instance with an explicit `DB_POOL_MAX`.
...
- Never pass a database row or type into a Server Component.
- Expose `withTransaction(work)` from the database aspect and pass its client to participating repositories.
...
Use the same order for `develop`. Keep every migration backward-compatible.
