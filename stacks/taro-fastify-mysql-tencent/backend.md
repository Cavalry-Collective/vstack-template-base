...
- **If you adopt the `multi-tenancy` add-on** (`add-ons/multi-tenancy/`): a `plugins/org.js` guard (`fastify.requireOrgMember(role)` as a `preHandler`, alongside `requireAuth`) resolves the `organisationId` path param, validates membership + organisation status, and packs the id into `request.ctx`; services and repos receive it as a named arg, and repos require it for every tenant-owned query. **This pack's `x-tenant` header is the test-mode signal, not organisational tenancy** — never resolve the organisation from it; `request.tenant` (mode) and `request.ctx.organisationId` (tenant) stay separate concepts. MySQL tables carry `organisation_id` (FK, indexed) with organisation-scoped composite uniques via Knex migrations; COS object keys are `organisations/{organisationId}/…`, read via signed URLs minted in `lib/cos.js` only after a membership re-check.
