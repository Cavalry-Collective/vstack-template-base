...
- **multi-tenancy** (`add-ons/multi-tenancy/`): a `shared/aspects/` preHandler resolves the `organisationId` path param, validates authenticated → member → organisation active, and passes the id inward on the request context as a value; repos take the organisation id as a required named argument, so an unscoped tenant query has no method to call. Tables carry `organisation_id` (FK, indexed) with organisation-scoped composite uniques via node-pg-migrate. Optional defence-in-depth: Postgres row-level security with `SET LOCAL` issued *inside* the transaction (the pooled endpoint is transaction-pooled — a session-level `SET` does not stick). Blob keys are `organisations/{organisationId}/…` behind an authorised read that re-checks membership.
