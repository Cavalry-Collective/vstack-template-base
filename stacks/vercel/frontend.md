...
- **Both paths validate responses against the shared Zod shape** before returning (base rule: a contract break surfaces as a typed error feeding the `error` state) — this is the frontend half of the pack README's "defined once, reused on both sides" schema rule.
...
- **Base says:** the base frames `apps/frontend` as a client-rendered SPA (`apps/frontend/CLAUDE.md` → *Versioning / build identity*: "A deployed SPA is cached"; and the `-spa` repo name). **In this stack:** it is server-first Next.js App Router on Vercel, not a client-rendered SPA. **Because:** the chosen stack. **Concretely:** default every component to server; add `'use client'` only at interaction leaves. (The repo name stays stale by design.)
