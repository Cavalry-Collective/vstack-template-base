...
This file owns the Nest module/ring mapping, DI, aspects, bootstrap, and the JS Babel setup. Persistence is **owned by `db/`** (see the db appendix) — this file covers only how Prisma is **consumed** in the repo ring, one kind of repo adapter, not the whole ring. Form-factor concerns live in the frontend appendix; backend pagination follows base unchanged.

## Stack binding at a glance

- **HTTP layer:** NestJS on the **Fastify** adapter (`@nestjs/platform-fastify`) — pack decision, see the pack README; Express is acceptable with a concrete reason, recorded there. **Persistence:** Prisma over PostgreSQL, owned by `db/` (see *Scope* above).
- **Language:** TypeScript **or** plain JS — not mandated. Rules are language-neutral with TypeScript as the default spine; JS-specific deltas are flagged `JS:` inline or collected in `## JavaScript path`.
...
- The pack provides its own `PrismaService` (see *Repo ring*), not the third-party `nestjs-prisma` package — pack decision, see the pack README.
...
- **Validation default: Zod** via a small custom `ZodValidationPipe` — pack decision, see the pack README (the shared schema language with the frontend is recorded there too). DTOs are Zod schemas in `dtos/`; infer the type in TS (`JS:` duck-typed).
...
- The single global exception filter is the **only** place that maps **domain errors → HTTP status** (reuse the base status-code table). **`domain/` and `service/` throw domain errors (plain classes), never Nest `HttpException`** — throwing it inward would leak transport concerns past the controller ring; `@nestjs/common` `HttpException` is **forbidden outside the controller ring and the filter** (see *Enforced boundaries*).
...
- **HTTP integrations** bind to `@nestjs/axios` `HttpService` (or a thin `fetch` wrapper) with **per-call timeout, bounded retry + backoff, and idempotency-key support**. Queue/scheduled work binds to BullMQ / `@nestjs/schedule`. The base **Integrations** rules (idempotency, concurrency, ordering, classified failures, untrusted-external handling) **apply unchanged**; the **decisions** they enforce stay in the domain.

## Request context / correlation id

- **`AsyncLocalStorage`** (recommend `nestjs-cls`), seeded by a Nest guard/interceptor — pack decision, see the pack README; the `app.use()` traps are in *Gotchas*. The seeder generates/propagates the correlation id (honour inbound `x-request-id`/`x-correlation-id`) into the ALS store.
...
The single import-boundary checklist (e.g. ESLint `no-restricted-imports` / `import/no-restricted-paths`); ring sections reference it rather than restate it. Verify a change against it:
...
- `main` entry: create the Fastify Nest app; register the **global exception filter**, the **global `ZodValidationPipe`**, **`@fastify/cookie`**, and the **correlation-id/logging path**. This bootstrap assumes the Nest CLI per app (build orchestrator — pack decision, see the pack README).
- **Cookies:** cookies are read/written at the **controller/aspect edge** via the Fastify request (through `@fastify/cookie`) and passed inward as plain values — never read by an inner ring.
...
- **Babel is required** so Nest decorators + DI metadata work without `tsc`. Prefer a known-good `babel.config` (or `nest-cli` with the **SWC** builder) over hand-ordering plugins. If hand-listing, in this order:
...
- **otp-auth** (`add-ons/otp-auth/`): model A (self-managed) — hashing + timing-safe verify in `shared/utils/`.
  - **Challenge store:** one `OtpChallenge` Prisma model (hashed code, short TTL, `purpose` discriminator); a unique constraint on (target, purpose) resolves the double-submit race to `409` per the base status table. Attempt rate limits keep their counters in Postgres through the module's repo ring (no separate store by default).
  - **Delivery & identity:** delivery gateways are repo-ring adapters behind domain ports, gated by the default-off flags; phone numbers canonicalised to E.164 with `libphonenumber-js`.
  - **Test mode:** delivery routes to the stdout adapter — the tester reads the real code there; verify is never stubbed.

## Gotchas

- **Babel plugin order matters:** `babel-plugin-transform-typescript-metadata` must run **before** the decorators plugin, or `design:type`/`design:paramtypes` never emit. `legacy` is an option of the decorators plugin (`{ legacy: true }`), not a separate plugin.
- **Omitting `babel-plugin-parameter-decorator`** (constructor-parameter DI) is the usual "Nest can't resolve dependencies" cause.
- **Don't seed request context with global `app.use()` middleware on Fastify:** it receives raw `req`/`res`, cannot access DI, and uses path-to-regexp wildcard syntax (`(.*)`, not `*`) — use the DI-capable guard/interceptor (see *Request context*).
- **Version-sensitive bindings** — the Babel recipe here, the Prisma config paths and `migrate diff` flags in the db appendix — assume the majors current when written; re-verify them on any major upgrade.

## Conflict register

- **Base says:** The stack is unchosen; the base presents JS-style filenames and an Express/Fastify-style HTTP layer **illustratively**, not as mandates. **In this stack:** that unchosen slot is filled with NestJS (a full framework) on the Fastify adapter; language relaxed to TS **or** JS. **Because:** Fastify was chosen on throughput + plugin ecosystem, not base continuity. **Concretely:** DON'T treat Nest as "a lightweight layer"; the JS path requires the `## JavaScript path` Babel setup.
- **Base says:** Wiring lives in `container.js`; manual constructor wiring, reach for a DI container only when the graph grows unwieldy. **In this stack:** each module's `providers` array is the composition root from day one — no `container.js`, no manual graph. **Because:** Nest's IoC container is mandatory infrastructure; a second container would duplicate it. **Concretely:** DO bind every port token to its adapter in the module `providers` array; DON'T add `container.js` or Awilix.
